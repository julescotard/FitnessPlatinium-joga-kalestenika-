...
def _shrink_payload(obj: Any, limit_items: int = 3) -> Any:
    if isinstance(obj, dict):
        out = {}
        for k, v in list(obj.items())[:60]:
            if isinstance(v, list):
                out[k] = _shrink_payload(v[:limit_items], limit_items)
            elif isinstance(v, dict):
                out[k] = _shrink_payload(v, limit_items)
            else:
                out[k] = v
        return out
    if isinstance(obj, list):
        return [_shrink_payload(x, limit_items) for x in obj[:limit_items]]
    return obj


def _filter_headers(h: Dict[str, str]) -> Dict[str, str]:
    drop = {"content-length", "host", "connection", "accept-encoding", "cookie"}
    return {k: v for k, v in (h or {}).items() if k.lower() not in drop}


def _set_date_fields(payload: Any, new_date: date) -> Any:
    """Heurystycznie podmienia pola daty w payloadzie na nową datę, zostawiając czas jeśli był."""
    def repl_value(v: Any) -> Any:
        if isinstance(v, str):
            s = v.strip()
            # ISO datetime?
            ...
            if m:
                tail = m.group(2) or ""
                return new_date.isoformat() + tail
            return v
        return v
    ...
    def rec(x: Any) -> Any:
        if isinstance(x, dict):
            ...
            for k, v in x.items():
                kl = k.lower()
                if "date" in kl or kl in {"day", "selectedday", "selecteddate"}:
                    out[k] = repl_value(v)
                else:
                    out[k] = rec(v)
            return out
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        for cal_url in cfg["club_calendars"]:
            ...
            # template for API iteration
            daily_template = {"url": None, "method": None, "headers": None, "payload": None}

            sample_payloads[cal_url] = {"filters": None, "daily_any": None, "daily_with_data": None, "daily_request_template": None}

            def on_request(req):
                try:
                    if "Classes/ClassCalendar/DailyClasses" in req.url and daily_template["payload"] is None:
                        daily_template["url"] = req.url
                        daily_template["method"] = req.method
                        daily_template["headers"] = req.headers
                        if req.method.upper() == "POST":
                            pd = req.post_data or ""
                            if pd.strip().startswith(("{","[")):
                                try:
                                    daily_template["payload"] = json.loads(pd)
                                except Exception:
                                    daily_template["payload"] = None
                except Exception:
                    pass
            ...
            def on_response(resp):
                try:
                    ...
                    if "GetCalendarFilters" in resp.url and sample_payloads[cal_url]["filters"] is None:
                        sample_payloads[cal_url]["filters"] = {"url": resp.url, "sample": _shrink_payload(obj, 3)}

                    if "DailyClasses" in resp.url:
                        if sample_payloads[cal_url]["daily_any"] is None:
                            sample_payloads[cal_url]["daily_any"] = {"url": resp.url, "sample": _shrink_payload(obj, 3)}
                        ...
                        if isinstance(cal, list) and len(cal) > 0 and sample_payloads[cal_url]["daily_with_data"] is None:
                            sample_payloads[cal_url]["daily_with_data"] = {"url": resp.url, "sample": _shrink_payload(obj, 3)}
                except Exception:
                    pass
            ...
            try:
                ...
                # gentle scroll: enough to trigger first API calls
                for _ in range(8):
                ...
                if daily_template["url"] and daily_template["method"]:
                    sample_payloads[cal_url]["daily_request_template"] = {
                        "url": daily_template["url"],
                        "method": daily_template["method"],
                        "headers": {k: v for k, v in (daily_template["headers"] or {}).items() if k.lower() in {"content-type","x-requested-with","requestverificationtoken"}},
                        "payload_keys": sorted(list(daily_template["payload"].keys())) if isinstance(daily_template["payload"], dict) else None,
                    }

                # === Key change: API iteration (fetch all days) ===
                api_events: List[Any] = []
                if daily_template["url"] and daily_template["method"] and daily_template["payload"] is not None:
                    headers = _filter_headers(daily_template["headers"] or {})
                    method = daily_template["method"].upper()
                    endpoint = daily_template["url"]

                    d = start_cutoff.date()
                    end_d = (end_cutoff - timedelta(days=1)).date()

                    while d <= end_d:
                        payload = _set_date_fields(copy.deepcopy(daily_template["payload"]), d)
                        try:
                            if method == "POST":
                                r = context.request.post(endpoint, data=json.dumps(payload), headers=headers, timeout=60000)
                            else:
                                # fallback
                                r = context.request.get(endpoint, headers=headers, timeout=60000)
                            if r.ok:
                                obj = r.json()
                                api_events.append(obj)
                                # stride optimization: if CalendarData includes multiple days, jump
                                stride = 1
                                if isinstance(obj, dict) and isinstance(obj.get("CalendarData"), list) and len(obj["CalendarData"]) > 1:
                                    stride = min(len(obj["CalendarData"]), 7)
                                d = d + timedelta(days=stride)
                            else:
                                d = d + timedelta(days=1)
                        except Exception:
                            d = d + timedelta(days=1)

                    captured_blobs.extend(api_events)
