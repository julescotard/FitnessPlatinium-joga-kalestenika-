...
NAME_KEYS = ["name", "title", "className", "activityName", "serviceName", "displayName", "activity"]
...
def _looks_like_event(d: Dict[str, Any]) -> bool:
    return bool(_extract_str(d, NAME_KEYS) and _extract_dt(d, DT_KEYS_START))
...
def _try_parse_json_text(txt: str) -> Optional[Any]:
    # anti-XSSI safe: strip everything before first { or [
    if not txt:
        return None
    t = txt.strip()
    if not t:
                ...
                for blob in captured_blobs:
                    for obj in _iter_json_objects(blob):
                        if not _looks_like_event(obj):
                            continue
                        name = _extract_str(obj, NAME_KEYS) or ""
                        name_n = _norm(name)
                        category = None
