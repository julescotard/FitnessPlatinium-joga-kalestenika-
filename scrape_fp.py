...
NAME_KEYS = ["name", "title", "className", "activityName", "serviceName", "displayName", "activity", "service", "classType"]
DT_KEYS_START = [
    "start", "startDate", "startDateTime", "startsAt", "dateFrom", "from", "begin", "date",
    "startUtc", "startUTC", "startTime", "startDateUtc", "startDateTimeUtc", "startDateTimeUTC"
]
DT_KEYS_END = [
    "end", "endDate", "endDateTime", "endsAt", "dateTo", "to", "finish",
    "endUtc", "endUTC", "endTime", "endDateUtc", "endDateTimeUtc", "endDateTimeUTC"
]
DATE_KEYS = ["date", "day", "classDate", "dateFrom", "startDate", "calendarDate"]
TIME_START_KEYS = ["startTime", "fromTime", "timeFrom", "hourFrom", "startHour"]
TIME_END_KEYS = ["endTime", "toTime", "timeTo", "hourTo", "endHour"]
...
def _extract_name(d: Dict[str, Any]) -> Optional[str]:
    for k in ["serviceName", "activityName", "className", "displayName", "title", "name"]:
        v = d.get(k)
        if isinstance(v, str) and v.strip():
            return v.strip()
    for k in ["service", "activity", "classType"]:
        v = d.get(k)
        if isinstance(v, dict):
            for kk in ["name", "title", "displayName"]:
                vv = v.get(kk)
...
def _write_ics(path: str, cal_name: str, events: List[Event], tz_name: str) -> None:
    ...
    lines: List[str] = [
        "BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//fp-smartplatinium-ics//PL","CALSCALE:GREGORIAN","METHOD:PUBLISH",
        f"X-WR-CALNAME:{_ics_escape(cal_name)}", f"X-WR-TIMEZONE:{tz_name}",
...
def main() -> None:
    ...
    if os.path.exists(prev_summary_path):
        try:
            prev_summary = json.load(open(prev_summary_path,"r",encoding="utf-8"))
        except Exception:
            prev_summary = {}
    ...
    if os.path.exists(prev_events_path):
        try:
            prev = json.load(open(prev_events_path,"r",encoding="utf-8"))
        ...
        for cal_url in cfg["club_calendars"]:
            ...
            def on_response(resp):
                try:
                    ...
                    def maybe_store_sample(obj: Any):
                        if cal_url not in sample_payloads and obj is not None:
                            sample_payloads[cal_url] = obj
            ...
            try:
                ...
                for blob in captured_blobs:
                    for obj in _iter_json_objects(blob):
                        ...
                        text_blob = _norm(name)
                        category = None
                        if any(k in text_blob for k in keywords["yoga"]):
                            category = "yoga"; yoga_any = True
                        elif any(k in text_blob for k in keywords["calisthenics"]):
                        ...
                        if end_dt:
                            if end_dt.tzinfo is None:
                                end_local = end_dt.replace(tzinfo=tz)
                            else:
                            ...
                            if t1:
                                end_local = datetime.combine(d1, t1).replace(tzinfo=tz)
                        ...
                        uid_seed = str(obj.get("id") or obj.get("classId") or obj.get("eventId") or f"{club_name}|{name}|{start_local.isoformat()}|{end_local.isoformat()}")
                ...
                tt = cal_url.split("timeTableId=")[-1]
                clubs_seen[f"{club_name} :: {tt}"] = {
                    "yoga": bool(yoga_any),
    ...
    for ev in all_events:
        st = ev.start_utc.astimezone(tz); en = ev.end_utc.astimezone(tz)
    ...
    json.dump(current_event_keys, open(os.path.join(args.out,"events_index.json"),"w",encoding="utf-8"), ensure_ascii=False, indent=2)

    cur_set = set(current_event_keys)
    added_events = sorted(cur_set - prev_event_keys)
    removed_events = sorted(prev_event_keys - cur_set)
    json.dump({"date": datetime.now(tz=tz).isoformat(timespec="seconds"), "added": added_events, "removed": removed_events},
              open(os.path.join(args.out,"events_changes.json"),"w",encoding="utf-8"),
    ...
    json.dump(changes, open(os.path.join(args.out,"changes.json"),"w",encoding="utf-8"), ensure_ascii=False, indent=2)
    ...
    json.dump(summary, open(os.path.join(args.out,"availability_summary.json"),"w",encoding="utf-8"), ensure_ascii=False, indent=2)

    json.dump(debug_calls, open(os.path.join(args.out,"debug_calls.json"),"w",encoding="utf-8"), ensure_ascii=False, indent=2)
    json.dump(sample_payloads, open(os.path.join(args.out,"sample_payloads.json"),"w",encoding="utf-8"), ensure_ascii=False, indent=2)
    ...
    md_lines = [
        ...
        "",
        "## Zmiany lokalizacji",
        f"- dodane: {len(changes['added'])}",
        f"- usunięte: {len(changes['removed'])}",
        "",
        "## Zmiany terminów",
        f"- nowe: {len(added_events)}",
        f"- usunięte: {len(removed_events)}",
        "",
    ]
    if errors:
        md_lines += ["## Błędy odczytu"] + [f"- {e}" for e in errors] + [""]

    open(os.path.join(args.out,"changes.md"),"w",encoding="utf-8").write("\\n".join(md_lines))
...
<li><a href="sample_payloads.json">sample_payloads.json</a></li>
    ...
    open(os.path.join(args.out,"index.html"),"w",encoding="utf-8").write(index_html)
